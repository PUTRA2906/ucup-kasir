              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="mt-2 space-y-1">
                  ...
                  <div class="flex items-center gap-1.5 text-xs">
                    ...
                    <span v-if="getEffectiveCreditLimit(customer) > 0" class="font-medium text-gray-900 dark:text-white">
                      {{ formatCurrency(getEffectiveCreditLimit(customer)) }}
                      <span v-if="usesDefaultLimit(customer)" class="text-[10px] font-normal text-gray-400 dark:text-gray-500">(default)</span>
                    </span>
                    <span v-else class="text-gray-400 dark:text-gray-600">Tanpa batas kredit</span>
                  </div>
                </div>
              </div>
...
const getEffectiveCreditLimit = (customer: Customer) => {
  return customer.credit_limit ?? settingsStore.settings.default_credit_limit ?? 0
}

const usesDefaultLimit = (customer: Customer) => {
  return customer.credit_limit === null || customer.credit_limit === undefined
}
